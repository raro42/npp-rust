...
3. Run **`./scripts/ci-local.sh`** (required — same as GitHub CI on this OS).
...
6. **On fail:** rename back to `WIP-…`, note the failure, leave `agent:wip`. Do not close the issue. Do **not** mark DONE if compile/test failed.
