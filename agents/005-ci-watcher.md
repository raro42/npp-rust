...
At least **once per UTC day**, inspect failing GitHub Actions CI on `main` / `dev` and queue a fix task when needed.
...
4. Push `dev`; fast-forward `main` when the tip should match.
